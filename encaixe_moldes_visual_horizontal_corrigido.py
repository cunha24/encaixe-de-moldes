...
st.title("Sistema de Encaixe Otimizado de Moldes - Visualização Horizontal")
...
if uploaded_file:
    ...
    for r in layout:
        rect = plt.Rectangle((-r['y'], r['x']), -r['comprimento'], r['largura'], edgecolor='black', facecolor='lightblue')
        ax.add_patch(rect)
        texto = f"{r['descricao']}\n{r['comprimento']}x{r['largura']}"
        ax.text(-r['y'] - r['comprimento']/2, r['x'] + r['largura']/2, texto, fontsize=6, ha='center', va='center')

    ax.set_xlim(-altura_total - 100, 0)
    ax.set_ylim(0, tecido_largura)
    ax.set_aspect('equal')
    st.pyplot(fig)
    ...
    with PdfPages(pdf_buffer) as pdf:
        ...
        pdf.close()
    st.download_button("Baixar Visualização em PDF", data=pdf_buffer.getvalue(), file_name="visualizacao_moldes_horizontal.pdf")
    ...
    if st.button("Exportar DXF"):
        ...
        for r in layout:
            ...
            texto = f"{r['descricao']}\n{r['comprimento']}x{r['largura']}"
            msp.add_text(texto, dxfattribs={'height': 5}).set_placement(
                (x0 + r['largura']/2, y0 - r['comprimento']/2), align='MIDDLE_CENTER'
            )
